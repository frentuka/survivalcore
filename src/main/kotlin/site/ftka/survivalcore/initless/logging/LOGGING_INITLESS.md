...
Administrators can dynamically query and read recent logs from disk, as well as live unwritten memory queues, directly inside Minecraft chat using the `ServerAdministrationApp` (`/server` command).

* **Command syntax:** `/server logs <loggerNames> [level] [date] [limit]`
* **Usage Example:** `/server logs PlayerDataService,PermissionsService error now 20`
* **Features:** 
  * Retrieves logs generated by multiple modules simultaneously, merging them into a perfectly chronologically sorted view.
  * Queries the live, not-yet-written `getBufferedLogs()` memory array seamlessly alongside disk `.json` files when the `now` parameter is provided.
  * Automatically color-codes module outputs using auto-generated acronym shortnames (e.g. `[PDS]`) with dynamic hovering.
...
### Log Model Schema
Using Kyori's `GsonComponentSerializer` mapped through a custom `LogSerializer` TypeAdapter, the output perfectly aligns with standard Vanilla Minecraft raw JSON components instead of ugly, backend java-reflection variables.

```typescript
interface LogJSON {
  color: string;       // e.g. "yellow"
  text: {              // Standard Minecraft JSON Text format
    text: string;
    color?: string;
    extra?: Array<any>;
  };
...
| **Swallowed IOException error** | System out of space or file locks | Inspect the Bukkit server console for severe prints from the plugin logger matching: `[SurvivalCore Logging]`. | Free disk space or close files locked by the OS editor. |
| **Queried logs are completely blank** | Attempting to deserialize a legacy log file format | Run the query again and observe if `[Error: Invalid Log Format]` appears in red. | Delete older `.json` logs generated prior to the `LogSerializer` component upgrade. |
